...
A2 е затворено засега, но Нина **може** да прави още корекции по-късно и ще
пуска нов PR. Затова редът е важен и не е по избор:

1. PR #15 влиза в master.
2. Нина се ре-синква **веднага** (`git merge origin/master` на клона `nina`) —
   fast-forward, без конфликти.
3. Фази 3–5 добавят `sectionStart` в **същите** `exercises.ts` файлове и влизат
   в master през PR C.
4. Нина се ре-синква **втори път, преди** да започне нови корекции.

Ако тя започне корекции преди стъпка 4, ще редактира версии на файловете без
разделите и мърджът ще даде конфликт във всеки пипнат `exercises.ts`. Конфликтът
е резолвим (нейното съдържание + запазен `sectionStart`), но е излишен —
достатъчно е Philip да ѝ каже да дръпне master, след като PR C е мърджнат.
...
| D7 | PR-и | Три отделни: **PR A** = мърдж (Фаза 1) · **PR B** = pretranslate (Фаза 2) · **PR C** = раздели + празненства (Фази 3–5). Фаза 2 и Фази 3–4 са **напълно независими** (`pretranslate.ts` не събира `sectionStart`) → може да вървят паралелно в два чата. |
| D8 | Кой измисля заглавията на разделите | **Агентът ги извежда от съдържанието на урока**, по модела на A1 — смислови теми, не етикети. Philip ревюва в браузъра (Фаза 3.5 и Фаза 6). Няма готов списък от UNHCR syllabus-а за разделите; syllabus-ът фиксира само имената на **уроците**, които не се пипат. |
| D9 | A2 урок 00 | **Третира се като нормален урок** — получава раздели и празненства. Изключването на `lesson-00` в A1 е специфично за Азбуката (няма нито раздели, нито Преговор); A2 урок 00 „Преговор A1“ е пълноценен урок с 32 упражнения. |
...
По D9 **всичките 11 урока влизат, включително `a2-lesson-00`** — той е
„Преговор A1“, пълноценен урок, не Азбука. Изключването на A1 `lesson-00` е
специфично за Азбуката и не се пренася.

Ако някой A2 урок няма workbook масив, последният му раздел става финалът —
това е коректно поведение, не бъг (`celebration.ts:196-206`). Провери само че
`buildCelebrationPlan` не връща празен план за урок 00.
