...
interface DialoguesProps {
  exercise: DialoguesExerciseShape;
...
export function Dialogues({
  ...
  const handleLineClick = (e: React.MouseEvent, section: DialogueSection, lineIndex: number) => {
    ...
    const audioFile = id ? `${id}-${section.id}-line-${lineIndex}` : '';
    const audioPath = audioFile
      ? getTtsAudioPath(id, 'dialogues', audioFile)
      : '';
    const spoken = stripBold(line.ttsText ?? line.text).replace(/^—\s*/, '');
