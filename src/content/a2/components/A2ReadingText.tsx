...
/**
 * A2 copy of the shared ReadingText (src/components/exercises/ReadingText.tsx).
 *
 * Why a copy: Nina's domain may not edit the shared exercise component. The ONLY
...
import { Play, Pause, BookOpen, Volume2, Turtle, Music } from 'lucide-react';
...
export function A2ReadingText({
  ...
  const ex = exercise as unknown as ReadingTextProps & { points?: number };
  // Mirror the built-in reading_text scoring (see ExerciseRenderer.wrapOneArgOnComplete).
  const scoreIfCorrect = ex.checklist?.items.length ?? ex.points ?? 1;
  return (
    <ReadingTextBase
      audioUrl={ex.audioUrl}
      songUrl={ex.songUrl}
      disableParagraphAudio={ex.disableParagraphAudio}
      ...
      hideText={ex.hideText}
      noTranslation={ex.noTranslation}
      checklist={ex.checklist}
      exerciseId={exerciseId ?? exercise.id}
      onComplete={onComplete ? (correct: boolean) => onComplete(correct, correct ? scoreIfCorrect : 0) : undefined}
    />
  );
}
