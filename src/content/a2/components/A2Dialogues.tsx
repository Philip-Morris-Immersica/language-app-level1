...
/**
 * A2 variant of the shared Dialogues (src/components/exercises/Dialogues.tsx).
 *
 * Why a copy: Nina's domain may not edit the shared exercise component, and the
 * shared one renders only ONE top-level image — it ignores per-`section` images.
 * Lesson 9 (and others) needs an image PER dialogue section (а., б., в.).
 *
 * Behavioural difference vs. the shared version: each `section.imageUrl` is
 * rendered, centered, above that section's lines (list layout). Everything else
 * (per-line TTS, whole-section playback, translations) is identical and uses the
 * same convention-based audio paths (keyed by `exerciseId` + `section.id`), so
 * pre-generated MP3s keep working.
 *
 * Registered as the opt-in type `'a2-dialogues'` in `../exercise-components.ts`
 * (A1 and other A2 lessons that use `'dialogues'` are NOT affected).
 *
 * SYNC: if the shared Dialogues changes, mirror the change here.
 */
...
interface A2DialoguesExerciseShape {
  id: string;
  type: string;
  subtitle?: string;
  imageUrl?: string;
        ...
        <div className="space-y-6">
          {sections.map((section) => {
            ...
            >
              ...
              {/* A2-CHANGE: render the per-section image (centered) above the lines. */}
              {section.imageUrl && (
                <div className="mb-4 flex justify-center">
                  <ImageLightbox src={section.imageUrl} alt={sectionTitle || section.id}>
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img
                      src={section.imageUrl}
                      alt={sectionTitle || section.id}
                      className="w-full max-w-xs md:max-w-sm rounded-xl object-cover shadow-sm"
                    />
                  </ImageLightbox>
                </div>
              )}
...
export function A2Dialogues({
  ...
  const ex = exercise as unknown as A2DialoguesExerciseShape;
  return (
    <DialoguesBase
      sections={ex.sections}
      imageUrl={ex.imageUrl}
      images={ex.images}
      displayLayout={ex.displayLayout}
