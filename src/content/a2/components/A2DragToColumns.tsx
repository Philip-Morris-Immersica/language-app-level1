...
/**
 * A2 variant of the shared `DragToColumns` (src/components/exercises/DragToColumns.tsx).
 *
 * Why a fork instead of a wrapper: the only change needed is deep inside the
 * per-item rendering (`ColumnDropZone`) — the "tap to remove" hover state uses
 * the red/error color family (`#FCE2DE` / `#D25A45`), which reads as "this
 * answer is wrong" even though it fires on EVERY placed item regardless of
 * correctness (feedback about correctness only appears after "Проверете").
 * That can't be changed via a prop on the shared component without editing
 * it, so this A2-only copy swaps that hover state to the neutral grey family
 * instead. Everything else is unchanged from the shared component.
 *
 * Registered as the opt-in type `'a2-drag-to-columns'` in `../exercise-components.ts`.
 */
...
interface A2DragToColumnsExerciseShape {
  imageUrl?: string;
  items: string[];
  points?: number;
  columns: {
          ...
          <div key={index} className="relative">
            <div
              className={`bg-white border border-gray-300 rounded-lg px-3 py-1.5 text-sm text-center font-medium text-gray-700 ${
                !submitted && onRemove ? 'cursor-pointer hover:bg-[#E5E5E5]/50 hover:border-[#737373]/50 active:scale-95 transition-all' : ''
              }`}
...
export function A2DragToColumns({
  exercise,
  onComplete,
}: {
  ...
  const ex = exercise as unknown as A2DragToColumnsExerciseShape;
  const { imageUrl, items, columns: columnConfig } = ex;
  const scoreIfCorrect = ex.points ?? items.length;
  ...
  const handleSubmit = () => {
    ...
    if (onComplete) {
      onComplete(allCorrect, allCorrect ? scoreIfCorrect : 0);
    }
  };
